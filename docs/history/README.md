...
- [`upstream-release-notes.txt`](upstream-release-notes.txt) — release highlights
  formerly stored in the Autotools-era `NEWS` file.
- [`upstream-faq.txt`](upstream-faq.txt) — GTK1-era FAQ with obsolete services,
  dependencies, and system advice.
- [`upstream-todo.txt`](upstream-todo.txt) — unfinished upstream ideas and
  references to the retired XMMS Bugzilla.
