...
static gint cddb_check_protocol_level(gchar *server)
{
	...
	{
		if (n > 0)
			cddb_log("Getting cddb protocol level failed: %s",
				 buffer);
		else
		...
		switch (command) {
			...
			case 2:
				...
				if (bufs > 0)
				{
					...
					{
						cdda_cdinfo_cd_set(cdinfo, g_strdup(temp+3), g_strndup(buffer2, temp-buffer2));
					}
					else
...
int search_for_discid(char *abs_filename,char **cddb_file,guint32 disc_id){
    ...
    while ((tmpdir = readdir(dir)) != NULL) {
        ...
        if (!strncmp(tmp_id,tmpdir->d_name,8)){
            if ((cddb_file[0] = (char *) g_malloc(strlen(abs_filename)+strlen(tmpdir->d_name)+1)) == NULL) {
                return(0);
            }
...
int scan_cddb_dir(char *server,char **cddb_file,guint32 disc_id){
    ...
    while (((tmp = readdir(dir)) != NULL)) {
        ...
        if ((tmp->d_name[0] != '.') && (stat(abs_filename,&st)!= -1) && (S_ISDIR(st.st_mode)) && (search_for_discid(abs_filename,cddb_file,disc_id))){
            break;
        }
