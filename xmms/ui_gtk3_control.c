...
{
	g_return_val_if_fail(XMMS_IS_UI_GTK3_CONTROL(control),
	...
	case GDK_BUTTON_PRESS:
		return xmms_ui_button_handle_pointer(&control->state,
			XMMS_UI_POINTER_PRESS, event->button.button, event->button.x,
			event->button.y);
	case GDK_BUTTON_RELEASE:
		return xmms_ui_button_handle_pointer(&control->state,
			XMMS_UI_POINTER_RELEASE, event->button.button, event->button.x,
			event->button.y);
	case GDK_MOTION_NOTIFY:
		return xmms_ui_button_handle_pointer(&control->state,
			XMMS_UI_POINTER_MOTION, 0, event->motion.x, event->motion.y);
	default:
